...
class LuaState {
  ...
  public:
    ...
    template <typename T, typename... Args>
    requires std::integral<T> || std::floating_point<T> std::vector<T> num_from_lua(int amount,
                                                                                    Args... args) {
    ...
    template <typename T>
    requires std::integral<T> || std::floating_point<T> std::vector<T>
    num_from_lua(const char* variable) {
