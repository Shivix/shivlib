...
constexpr decltype(auto) apply_impl(FuncT&& function, TupleT&& tuple, std::index_sequence<seq...>) {
    return shiv::invoke(shiv::forward<FuncT>(function),
                        std::get<seq>(shiv::forward<TupleT>(tuple))...);
}
template <typename FuncT, typename TupleT>
constexpr decltype(auto) apply(FuncT&& function, TupleT&& tuple) {
    return apply_impl(
        shiv::forward<FuncT>(function), shiv::forward<TupleT>(tuple),
        std::make_index_sequence<std::tuple_size_v<shiv::remove_reference_t<TupleT>>>{});
}
